 ...
 *
 * The input of [string] is unchecked.
 */
@Deprecated(
    level = DeprecationLevel.ERROR,
    message = "Hex moved to internal API.",
    replaceWith = ReplaceWith("fromHex(string)")
)
fun hex(string: String): ByteArray = fromHex(string)

/**
 * Decode bytes from HEX [string]. It should be no spaces and `0x` prefixes.
 *
 * The input [string] is unchecked.
 */
@InternalAPI
fun fromHex(string: String): ByteArray {
    ...
    for (idx in 0 until result.size) {
        val srcIdx = idx * 2
        val high = string[srcIdx].toString().toInt(16) shl 4
        val low = string[srcIdx + 1].toString().toInt(16)
